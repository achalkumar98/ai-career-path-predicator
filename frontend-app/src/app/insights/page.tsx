...
export default function Insights() {
  ...
  const highlights = [
    { icon: Brain, label: 'Personality Analysis', desc: 'Understand your work style and strengths' },
    ...
    <div style={{ minHeight: 'calc(100vh - 56px)', background: '#f9fafb' }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #fffbeb 0%, #f9fafb 60%)', borderBottom: '1px solid #e5e7eb', padding: '64px 48px', textAlign: 'center' }}>
        ...
        <h1 style={{ fontSize: '42px', fontWeight: 800, color: '#0f1729', lineHeight: 1.15, marginBottom: '16px' }}>
          Personality &<br /><span style={{ color: '#d97706' }}>Trends</span>
        </h1>
        <p style={{ fontSize: '16px', color: '#6b7280', maxWidth: '520px', margin: '0 auto 32px', lineHeight: 1.7 }}>
          Describe yourself and get AI-powered personality analysis, career trend insights, and tailored recommendations based on who you are.
        </p>
        <button
          onClick={() => { setOpen(true); setResult(null); }}
          style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '13px 28px', borderRadius: '10px', background: '#d97706', color: '#fff', fontSize: '14px', fontWeight: 600, border: 'none', cursor: 'pointer', boxShadow: '0 4px 14px rgba(217,119,6,0.3)', transition: 'background 150ms' }}
      ...
      <div style={{ padding: '48px', maxWidth: '900px', margin: '0 auto' }}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '20px' }}>
          {highlights.map(({ icon: Icon, label, desc }) => (
            ...
            >
              <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: '#fffbeb', display: 'flex', alignItems: 'center', justifyContent: 'center', marginBottom: '14px' }}>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', width: '100%', maxWidth: '520px', boxShadow: '0 20px 40px rgba(0,0,0,0.15)', overflow: 'hidden' }}>
            <div style={{ padding: '20px 24px', borderBottom: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <div>
                  <p style={{ fontSize: '14px', fontWeight: 700, color: '#0f1729' }}>Personality & Trends</p>
            ...
            <div style={{ padding: '24px' }}>
              {!result ? (
                <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                  ...
                  <button type="submit" disabled={loading} style={{ width: '100%', padding: '11px', borderRadius: '10px', background: loading ? '#fbbf24' : '#d97706', color: '#fff', fontSize: '13px', fontWeight: 600, border: 'none', cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
                    {loading ? <><Loader2 size={14} style={{ animation: 'spin 1s linear infinite' }} /> Analyzing...</> : 'Get My AI Insight →'}
                  </button>
                </form>
              ) : (
                <div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px' }}>
                    <Sparkles size={15} style={{ color: '#d97706' }} />
                    <p style={{ fontSize: '13px', fontWeight: 600, color: '#0f1729' }}>Your AI Career Insight</p>
                  </div>
                  ...
                  <button onClick={() => { setResult(null); setUserInput(''); }} style={{ marginTop: '16px', width: '100%', padding: '10px', borderRadius: '8px', border: '1px solid #e5e7eb', background: '#fff', fontSize: '13px', cursor: 'pointer', color: '#374151' }}>
                    Try Again
                  </button>
                </div>
              )}
