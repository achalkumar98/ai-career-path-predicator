  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f9fafb', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '56px 48px', display: 'grid', gridTemplateColumns: '1fr 1.6fr', gap: '48px', alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            {info.map(({ icon: Icon, label, value }) => (
              <div key={label} style={{ display: 'flex', alignItems: 'flex-start', gap: '14px' }}>
                <div style={{ width: '40px', height: '40px', borderRadius: '10px', background: 'rgba(34,85,236,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                  <Icon size={17} style={{ color: '#2255ec' }} />
                </div>
                <div>
                  <p style={{ fontSize: '11px', fontWeight: 600, color: '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '2px' }}>{label}</p>
        ...
        {/* Right — form */}
        <div style={{ background: '#fff', borderRadius: '16px', padding: '36px', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid #e5e7eb' }}>
          {sent ? (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <CheckCircle size={48} style={{ color: '#059669', margin: '0 auto 16px' }} />
              <h3 style={{ fontSize: '18px', fontWeight: 700, color: '#0f1729', marginBottom: '8px' }}>Message sent!</h3>
              <p style={{ fontSize: '13px', color: '#6b7280', marginBottom: '24px' }}>Thanks for reaching out. We&apos;ll get back to you within 24 hours.</p>
            ...
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                {[
                  ...
                  <div key={f.id}>
                    <label htmlFor={f.id} style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>{f.label}</label>
                    <input id={f.id} type={f.type} placeholder={f.placeholder} value={f.val} onChange={e => set(f.key, e.target.value)} required
              ...
              <div>
                <label htmlFor="subject" style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Subject</label>
                <input id="subject" type="text" placeholder="How can we help?" value={form.subject} onChange={e => set('subject', e.target.value)} required
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '13px', color: '#0f1729', outline: 'none', boxSizing: 'border-box', transition: 'border-color 150ms' }}
                  onFocus={e => (e.currentTarget.style.borderColor = '#2255ec')}
                  onBlur={e => (e.currentTarget.style.borderColor = '#e5e7eb')} />
              </div>
              ...
              <div>
                <label htmlFor="message" style={{ display: 'block', fontSize: '12px', fontWeight: 500, color: '#374151', marginBottom: '6px' }}>Message</label>
                <textarea id="message" placeholder="Tell us more about your question or feedback..." value={form.message} onChange={e => set('message', e.target.value)} required rows={5}
                  style={{ width: '100%', padding: '10px 12px', borderRadius: '8px', border: '1px solid #e5e7eb', fontSize: '13px', color: '#0f1729', outline: 'none', boxSizing: 'border-box', resize: 'vertical', fontFamily: 'inherit', transition: 'border-color 150ms' }}
                  onFocus={e => (e.currentTarget.style.borderColor = '#2255ec')}
                  onBlur={e => (e.currentTarget.style.borderColor = '#e5e7eb')} />
              </div>
