...
module.exports = (schema) => (req, res, next) => {
  ...
  try {
    if (schema.body) schema.body.validateAsync(req.body).catch((e) => { throw e; });
    if (schema.query) schema.query.validateAsync(req.query).catch((e) => { throw e; });
    if (schema.params) schema.params.validateAsync(req.params).catch((e) => { throw e; });
